...
void HapMuC::getReadsFromBams(vector<MyBam *> & Bams, uint32_t leftPos, uint32_t rightPos, vector<Read> & reads, uint32_t & oldLeftPos, uint32_t & oldRightFetchReadPos, vector<Read *> & readBuffer, const bool reset)
{
   ...
   //   cout << " __reads[" << r  << "]: " << bam1_qname(reads[r].getBam()) << " UNMAPPED" << endl;
	...
	for (int r=0;r<int(reads.size());r++) {
		...
		//cout << "reads.mapQual " << reads[r].mapQual <<  " mq: " << reads[r].getBam()->core.qual << endl;
...
#ifdef NDEBUG
    cout << "reads[" << max << "]: " << reads[max].seq_name << "pos: " << reads[max].pos << " matePos: " << reads[max].matePos << " mateLen: " << reads[max].mateLen  <<  " mq: " << reads[max].mapQual << endl;
#endif
...
#ifdef NDEBUG
            cout << "##################################" << endl;
            cout << "normal reads from getReadsFromBams" << endl;
            BOOST_FOREACH(Read r, normalReads) {
              cout << r.seq_name << endl;
            }
            cout << "##################################" << endl;
            cout << "tumor reads from getReadsFromBams" << endl;
            BOOST_FOREACH(Read r, tumorReads) {
              cout << r.seq_name << endl;
            }
#endif
