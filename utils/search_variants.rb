...
open(tumor_pileup) { |tf|
  open(normal_pileup) { |nf|
    ...
    while tl = tf.gets
      ...
      count += 1
      if count % 100000 == 0
        warn [count, tr.chr, tr.pos]
        GC.start
      end
      while true
        warn [tr, nr] if $DEBUG
        if !nr.nil? and (tr.chr == nr.chr and tr.pos == nr.pos)
          break unless tr.depth >= @tumor_depth_th
          break unless nr.depth >= @normal_depth_th
          break if tr.bases.gsub(/(\^.)|[\.,\$]/,'').length < @min_variant_num
          warn "*" * 20 if $DEBUG
          warn tr if $DEBUG
          warn nr if $DEBUG
          break if tr.ref == "N"
          ...
          break
        elsif nr.nil? or (tr.chr == nr.chr and tr.pos > nr.pos)
          nl = nf.gets
          break if nl.nil?
          nr = Record.new(nl)
          nchr = nr.chr
